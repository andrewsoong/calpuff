 ...
 * BALDAT  =  massbald.dat  *
 ...
 ! LCFILES = T! Pasar todo a lowercase:
 
 ! END !
 --------------------------------------------------------------------------------
 1 -- General run control parameters
 
 	! METRUN =0 !	correr todo el periodo de la meteo (1) ó el definido abajo (0):
	...
	! XBTZ  = ${time_offset} !
 	...
 	Run parameters
 	! ITEST    =2    !	1: parar desp de setup, 2: seguir ejercución 
 	! MRESTART =0    !	restart [0/1]
 	! NRESPD   =0    !

 	! METFM    =1    !	formato de meteo. (1:calmet)
 	! MPRFFM   =1    !	formato de perfil meteo.

	! AVET     =60.0 ! tiempo de promediado (min)
 	! PGTIME   =60.0 ! tiempo promedio de sigmas
 ...
 2 -- Technical options
 	...
 	! MGAUSS   =1 !   distr vertical : 0:uniforme; 1:gaussiana.
 	! MCTADJ   =3 !   metodo de ajuste terreno: 0:no;1:ISC; 2:simple calpuff; 3: partial plume path adjusment.
 	! MTRANS   =1 !   transitional rise plume model? [0/1]
 	! MTIP     =1 !   stack tip down-wash? [0/1]
 	
 	! MCHEM    =0  ! mecanismo quimico: 0:no  1: mesopuff  2: user-defined 3: rivad/arm3
 	! MWET     =1  ! wet removal? [0/1]
 	! MDRY     =1  ! dry removal? [0/1]
 	! MDISP    =3  ! metodo para computar sigmas.
 	! MROUGH   =0  ! ajustar sigmas por rugosidad
 	! MPARTL   =1  ! considerar pluma que penetra en inversion
 ...
 5 -- Output Options
 	...
 	! ICON =1 !     crear  conc.dat [0/1]
 	! IDRY =1 !     crear  dflx.dat [0/1]
 	! IWET =1 !     crear  wflx.dat [0/1]
 	! IVIS =1 !     crear  vis.dat [0/1]
 	! IT2D =0 !    mostrar T en estaciones de superficie
 	! IRHO =0 !    mostrar Rho en estaciones de superficie
 	
 	! LCOMPRS =F !  data compression?
...
echo -e "Revisar calpuff.inp y si todo ok: $> ./calpuff.exe"
